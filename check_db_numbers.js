...
async function check() {
  ...
  const inc = await client.query(`SELECT sum(amount) FROM "Income" WHERE date >= $1 AND date <= $2 AND currency='ARS'`, [startDate, endDate]);
  const exp = await client.query(`SELECT sum(amount) FROM "Expense" WHERE date >= $1 AND date <= $2 AND currency='ARS'`, [startDate, endDate]);
  ...
  const invs = await client.query(`SELECT sum(amount) FROM "Investment" WHERE "startDate" >= $1 AND "startDate" <= $2 AND currency='ARS'`, [startDate, endDate]);
  ...
  const tInvs = Number(invs.rows[0].sum || 0);

  console.log('Balance:', tInc - tExp - tSDep + tSRet - tInvs);
